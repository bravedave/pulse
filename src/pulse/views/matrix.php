    ...
    (_ => {
      ...
      const matrix = data => {
        ...
        $.each(data, (i, dto) => {
          ...
          const randomString = _.randomString();

          const row = $(`<article class="card mb-4 pointer" data-id="${dto.id}">
              <div class="card-header">
                <div class="form-check float-end">
                  <input type="checkbox" class="form-check-input"
                    name="seen" id="${randomString}"
                    data-id="${dto.id}"
                    ${!!dto.seen ? 'checked' : ''}>
                  <label class="form-check-label" for="${randomString}">
                    seen
                  </label>
                </div>
          ...
          row.find('.form-check').on('click', e => e.stopPropagation());
          row.find('input[type="checkbox"][name="seen"]')
